...
namespace Demo.Framework.Core.Security
{
    ...
    {
        private static byte[] CreateBsKey(string key, int len)
        {
            byte[] bsKey = new byte[len];
            if (key.Length >= len)
            {
                bsKey = Encoding.UTF8.GetBytes(key.Substring(0, len));
            }
            else
            {
        ...
        /// <returns></returns>
        public static string DesEncrypt(string key, string source)
        {
            byte[] bsIV = { 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF };
            byte[] bskey = CreateBsKey(key, bsIV.Length);

            DESCryptoServiceProvider des = new DESCryptoServiceProvider();
            byte[] inputByteArray = Encoding.UTF8.GetBytes(source);
            MemoryStream ms = new MemoryStream();
            CryptoStream cs = new CryptoStream(ms, des.CreateEncryptor(bskey, bsIV), CryptoStreamMode.Write);
            cs.Write(inputByteArray, 0, inputByteArray.Length);
            cs.FlushFinalBlock();
            string re = Convert.ToBase64String(ms.ToArray());
            ms.Close();
            return re;
        }
        ...
        /// <returns></returns>
        public static string DesDecrypt(string key, string destring)
        {
            ...
            DESCryptoServiceProvider des = new DESCryptoServiceProvider();
            byte[] source = Convert.FromBase64String(destring);

            MemoryStream ms = new MemoryStream();
            CryptoStream cs = new CryptoStream(ms, des.CreateDecryptor(bsKey, bsIV), CryptoStreamMode.Write);
        ...
        /// <param name="source"></param>
        /// <returns></returns>
        public static string RC2Encrypt(string key, string source)
        {
            byte[] bsIV = { 0x12, 0x34, 0x56, 0x78, 0x90, 0xAB, 0xCD, 0xEF };
            byte[] bskey = CreateBsKey(key, bsIV.Length);
            ...
            byte[] inputByteArray = Encoding.UTF8.GetBytes(source);
            MemoryStream ms = new MemoryStream();
            CryptoStream cs = new CryptoStream(ms, rc2.CreateEncryptor(bskey, bsIV), CryptoStreamMode.Write);
            cs.Write(inputByteArray, 0, inputByteArray.Length);
            cs.FlushFinalBlock();
            string re = Convert.ToBase64String(ms.ToArray());
            ms.Close();
        ...
        /// <param name="destring"></param>
        /// <returns></returns>
        public static string RC2Decrypt(string key, string destring)
        {
            ...
            MemoryStream ms = new MemoryStream();
            CryptoStream cs = new CryptoStream(ms, rc2.CreateDecryptor(bsKey, bsIV), CryptoStreamMode.Write);
        ...
        public static void RSACreateKey(ref string str_PrivateKey, ref string str_PublicKey)
        {
        ...
        {
            RSACryptoServiceProvider rsa = new RSACryptoServiceProvider();
            //System.Security.Cryptography.RSA.FromXmlString(str_PublicKey);
        ...
        {
            using (var fs = new FileStream(filename, FileMode.Open, FileAccess.Read))//打开文件
            {
                MD5 md5 = MD5.Create();
